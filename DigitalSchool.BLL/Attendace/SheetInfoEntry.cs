    ...
    public class SheetInfoEntry : IDisposable
    {
                ...
                for (int i = 0; i < dt.Rows.Count; i++)
                {
                    ...
                    while (BeginDate <= EndDate)
                    {
                       // string[] Dates = BeginDate.ToString().Split('/');
                        ...
                        string datTime = BeginDate.ToString("yyyy-MM-dd")+" " + DateTime.Now.ToString("HH:mm:ss");
                        DateTime AttDateTime = DateTime.Parse(datTime); //DateTime.ParseExact(datTime, dateFormat, CultureInfo.InvariantCulture);
                        ...
                        result = CRUD.ExecuteQuery("insert into DailyAttendanceRecord (StudentId,RollNo,BatchId,ShiftId,ClsSecId,ClsGrpId,AttDate,AttStatus,AttManual,InHur,InMin,InSec,OutHur,OutMin,OutSec,DailyStartTimeALT_CloseTime) " +
                             " values (" + dt.Rows[i]["StudentId"].ToString() + "," + dt.Rows[i]["RollNo"].ToString() + "," + Batch + "," + Shift + "," + section + "," + group + ",'" + AttDateTime.ToString("yyyy-MM-dd HH:mm:ss") + "',' ',' ', " +
        ...
        public static DataTable  HasAttendanceMonthDateInDATable(string Shift,string Batch,string group,string section,string MonthYear,string Day)
        {
            try
            {
                ...
                dt = CRUD.ReturnTableNull("select Distinct StudentId,RollNo,FullName,convert(varchar(10),AdmissionDate,105) as AdmissionDate from v_CurrentStudentInfo_ForAttendance where ConfigId=" + Shift + " AND BatchId=" + Batch + " AND ClsGrpID=" + group + "  AND ClsSecID=" + section + " AND " +
                    " AdmissionDate<='" +MY[1]+"-"+MY[0]+"-" + DaysInMonth + "'  AND IsActive='true' AND StudentId not in " +
                    " (select distinct StudentId from DailyAttendanceRecord where right( convert(varchar(10), AttDate,105),7)='" + MonthYear + "' AND StudentId is not null ) ");
        ...
        public static DataTable loadAttendanceSheetByMonthYear(string months_year, string BatchId, string ShiftId, string ClsSecId, string ClsGrpId,string AdmissionNo)
        {
            try
            {
