 ...
 public static   class ForAttendanceReport
    {
     ...
     public static DataTable returntDataTableForAttSheet(string ShifId, string BatchId, string ClsGrpId, string ClsSecId, string MonthId, string StudentId) // For Monthly Student Attendance Sheet
     {
         try
         {
             CurrentStdEntry currentstdEntry = new CurrentStdEntry(); ;
             string condition = "";          
           
            
             condition = currentstdEntry.GetSearchCondition2(ShifId, BatchId, ClsGrpId, ClsSecId);
             if(condition=="")
             {
                 if(StudentId=="All")
                 {
                     condition = " where format(attdate,'MM-yyyy')='" + MonthId + "'";
                 }
                 else
                 {
                     condition = " where format(attdate,'MM-yyyy')='" + MonthId + "' and StudentId='" + StudentId + "'";
                 }
             }
             else
             {
                 if (StudentId == "All")
                 {
                     condition += " and format(attdate,'MM-yyyy')='" + MonthId + "'";
                 }
                 else
                 {
                     condition += " and format(attdate,'MM-yyyy')='" + MonthId + "' and StudentId='" + StudentId + "'";
                 }
             }
             ...
             sqlcmd = "SELECT   FullName, RollNo, SUM(CASE DATEPART(day, AttDate) WHEN 1 THEN code ELSE 0 END) AS [1], SUM(CASE DATEPART(day, AttDate)"+ 
                      ...
                      " FROM  dbo.v_DailyAttendanceRecordForReport  "+condition+"" +
     ...
     public static DataTable return_dt_for_AttSummary(string FDate, string TDate, string BatchId, string ShiftId, string ClsGrpId, string ClsSecId, string StudentId) // For Student's Attendance Summary 
     {
         try{
             ...
             condition = currentstdEntry.GetSearchCondition3(ShiftId, BatchId, ClsGrpId, ClsSecId);
             if (condition == "")
             {
                 if (StudentId == "All")
                 {
                     condition = " where Convert(datetime,AttDate,105) between Convert(datetime,'" + FDate + "',105) AND Convert(datetime,'" + TDate + "',105)";
                 }
                 else
                 {
                     condition = " where Convert(datetime,AttDate,105) between Convert(datetime,'" + FDate + "',105) AND Convert(datetime,'" + TDate + "',105) and DailyAttendanceRecord.StudentId='" + StudentId + "'";
                 }
             }
             else
             {
                 if (StudentId == "All")
                 {
                     condition += " and Convert(datetime,AttDate,105) between Convert(datetime,'" + FDate + "',105) AND Convert(datetime,'" + TDate + "',105)";
                 }
                 else
                 {
                     condition += " and Convert(datetime,AttDate,105) between Convert(datetime,'" + FDate + "',105) AND Convert(datetime,'" + TDate + "',105) and DailyAttendanceRecord.StudentId='" + StudentId + "'";
                 }
             }
     ...
     public static DataTable return_dt_for_EmpAttSummary(string Shift,string DId,string DesId,string FDate,string TDate,string empType, string EId) //For Staff & Faculty Att Summary 
     {
         try {
             ...
             sqlcmd="SELECT DISTINCT "+
                ...
                " SUM(CAST(CASE WHEN ATTStatus = 'lv' THEN 1 ELSE 0 END AS int)) AS [Total Leave], dbo.EmployeeInfo.ECardNo as RollNo, dbo.EmployeeInfo.EName as FullName, dbo.Departments_HR.DId, dbo.Designations.DesId, dbo.Departments_HR.DName, dbo.Designations.DesName " +
                ...
                " dbo.Departments_HR ON dbo.DailyAttendanceRecord.DId = dbo.Departments_HR.DId INNER JOIN " +
                ...
                " GROUP BY dbo.DailyAttendanceRecord.EID,DailyAttendanceRecord.ShiftId,ShiftConfiguration.ShiftName, dbo.EmployeeInfo.ECardNo, dbo.EmployeeInfo.EName, dbo.Departments_HR.DId, dbo.Designations.DesId, dbo.Departments_HR.DName, dbo.Designations.DesName ";
             else
             sqlcmd = "SELECT DISTINCT " +
               " dbo.DailyAttendanceRecord.EID,DailyAttendanceRecord.ShiftId,ShiftConfiguration.ShiftName, SUM(CAST(CASE WHEN ATTStatus = 'p' THEN 1 ELSE 0 END AS int)) AS [Total Present], SUM(CAST(CASE WHEN ATTStatus = 'a' THEN 1 ELSE 0 END AS int)) AS [Total Absent], " +
               " SUM(CAST(CASE WHEN ATTStatus = 'lv' THEN 1 ELSE 0 END AS int)) AS [Total Leave], dbo.EmployeeInfo.ECardNo as RollNo, dbo.EmployeeInfo.EName as FullName, dbo.Departments_HR.DId, dbo.Designations.DesId, dbo.Departments_HR.DName, dbo.Designations.DesName " +
               ...
               " dbo.Designations ON dbo.DailyAttendanceRecord.DesId = dbo.Designations.DesId INNER JOIN " +
               " dbo.ShiftConfiguration ON dbo.DailyAttendanceRecord.ShiftId = dbo.ShiftConfiguration.ConfigId " +
               " where " + Shift + "  dbo.DailyAttendanceRecord.Eid='" + EId + "' " +
     ...
     public static DataTable return_dt_for_EmpAttSummary(string FDate, string TDate, string EId) //For Staff & Faculty Att Summary 
     {
         try
         {
                 sqlcmd = "SELECT DISTINCT " +
                   ...
                   " dbo.Designations ON dbo.DailyAttendanceRecord.DesId = dbo.Designations.DesId INNER JOIN " +
                   " dbo.ShiftConfiguration ON dbo.DailyAttendanceRecord.ShiftId = dbo.ShiftConfiguration.ConfigId " +
                   " where  dbo.DailyAttendanceRecord.Eid='" + EId + "' " +
     ...
     public static DataTable return_dt_AbsentDetails(string FDate, string TDate,string ShiftId,string ClsGrpId,string ClsSecId, string StudentId) // For Student's Absent Details Records
     {
         try
         {
             ...
             condition = currentstdEntry.GetSearchCondition3(ShiftId, "All", ClsGrpId, ClsSecId);
             if (StudentId == "All")
             {
                 if(condition=="")
                 {
                     condition = "where AttStatus='A'  and Convert(datetime,AttDate,105) between Convert(datetime,'" + FDate + "',105) AND Convert(datetime,'" + TDate + "',105)";
                 }
                 else
                 {
                     condition += " and AttStatus='A'  and Convert(datetime,AttDate,105) between Convert(datetime,'" + FDate + "',105) AND Convert(datetime,'" + TDate + "',105)";
                 }
     ...
     public static DataTable return_dt_DailyEmpAttReport(string reportType, string DepartmentList, string DesignationList, string shift, string FDate,string TDate,string empType,string Eid) //Daily Attendance of Staff & Faculty 
     {
         ...
         shift = (shift == "0") ? "" : "and ShiftId='" + shift + "'";
             ...
             {
                 if(Eid=="0")
                     sqlcmd = "select ECardNo,EName,DName,DesName,ShiftName,ShiftId,AttStatus,Format(AttDates,'dd-MM-yyyy') as AttDates,Format(AttDates,'MM-yyyy') as AttMonth from v_DailyEmployeeAttendanceRecord where DId in(" + DepartmentList + ") and DesId in(" + DesignationList + ") " + shift + "  and Convert(datetime,AttDate,105) between Convert(datetime,'" + FDate + "',105) AND Convert(datetime,'" + TDate + "',105) " + empType + " ORDER BY ShiftName,DName,ECardNo";
                 else
                     sqlcmd = "select ECardNo,EName,DName,DesName,ShiftName,AttStatus,ShiftId,Format(AttDates,'dd-MM-yyyy') as AttDates,Format(AttDates,'MM-yyyy') as AttMonth from v_DailyEmployeeAttendanceRecord where EId='" + Eid + "' " + shift + "  and Convert(datetime,AttDate,105) between Convert(datetime,'" + FDate + "',105) AND Convert(datetime,'" + TDate + "',105) ORDER BY ShiftName,DName,ECardNo ";
             }
             ...
             {  
                 if(Eid=="0")
                     sqlcmd = " SELECT ECardNo,EName, DName, DesName,ShiftName,ShiftId,AttStatus, InHur, InMin, OutHur,OutMin,Format (AttDates,'dd-MM-yyyy') as AttDates,Format(AttDates,'MM-yyyy') as AttMonth" +
                         ...
                         " where DId in(" + DepartmentList + ") and DesId in(" + DesignationList + ") " + shift + " and Convert(datetime,AttDate,105) between Convert(datetime,'" + FDate + "',105) AND Convert(datetime,'" + TDate + "',105) " + empType + "" +
                          "ORDER BY ShiftName,DName,ECardNo";
                 else
                     sqlcmd = " SELECT ECardNo,EName, DName, DesName,ShiftName,ShiftId,AttStatus, InHur, InMin, OutHur,OutMin,Format (AttDates,'dd-MM-yyyy') as AttDates,Format(AttDates,'MM-yyyy') as AttMonth" +
                       " FROM  v_DailyEmployeeAttendanceRecord" +
         ...
         try
         {
             ...
             if(EId=="0")
                 sqlcmd = " SELECT  EName as FullName, ECardNo as RollNo,ShiftName,ShiftId,  SUM(CASE DATEPART(day, AttDates) WHEN 1 THEN code ELSE 0 END) AS [1], SUM(CASE DATEPART(day, AttDates) " +
                       ...
                       " GROUP BY EName,ECardNo,DName,ShiftName,ShiftId ORDER BY ShiftName,ECardNo";
             else
                 sqlcmd = " SELECT  EName as FullName, ECardNo as RollNo,ShiftName,ShiftId,  SUM(CASE DATEPART(day, AttDates) WHEN 1 THEN code ELSE 0 END) AS [1], SUM(CASE DATEPART(day, AttDates) " +
                       " WHEN 2 THEN code ELSE 0 END) AS [2], SUM(CASE DATEPART(day, AttDates) WHEN 3 THEN code ELSE 0 END) AS [3], SUM(CASE DATEPART(day, AttDates) WHEN 4 THEN code ELSE 0 END) AS [4], " +
                       " SUM(CASE DATEPART(day, AttDates) WHEN 5 THEN code ELSE 0 END) AS [5], SUM(CASE DATEPART(day, AttDates) WHEN 6 THEN code ELSE 0 END) AS [6], SUM(CASE DATEPART(day, AttDates) " +
     ...
     public static void GetRollNoBySection(DropDownList dl, string shiftID, string batchID, string clsgrpID, string clsSecId)
     {
         if (clsSecId == "All")
         {
             sqlcmd = string.Format("Select distinct RollNo, StudentId from DailyAttendanceRecord "+
                 " WHERE ShiftId='"+shiftID+"' AND  BatchID='"+batchID+"' AND ClsGrpID='"+clsgrpID+"'  order by RollNo ");
         }
         else
         {
             sqlcmd = string.Format("Select distinct RollNo, StudentId from DailyAttendanceRecord " +
                 " WHERE ShiftId='" + shiftID + "' AND  BatchID='" + batchID + "' AND ClsGrpID='" + clsgrpID + "' AND ClsSecID='" + clsSecId + "' order by RollNo");
         }
         dt = CRUD.ReturnTableNull(sqlcmd);
         dl.DataSource = dt;
     ...
     public static void LoadCardNo_ForAtt(DropDownList dl, string DepartmentList, string DesignationList, string ShiftId, string empType)  // For Load Staff & Faculty Card No.
     {
         empType = (empType == "2") ? "" : "and IsFaculty=" + empType + "";
         ShiftId = (ShiftId == "0") ? "" : " and ShiftId='" + ShiftId + "'";
         DataTable dt = new DataTable();
         dt = CRUD.ReturnTableNull(" select distinct EID,CONVERT(varchar(30),ECardNo)+'_'+EName as ECardNo from v_DailyEmployeeAttendanceRecord where DId in(" + DepartmentList + ") and DesId in(" + DesignationList + ") " + ShiftId + " " + empType + "");
         dl.DataSource = dt;
     ...
     public static DataTable returnTodaysAttendanceStatus()  // For Load Staff & Faculty
     {
         
         DataTable dt = new DataTable();
         dt = CRUD.ReturnTableNull("select ei.EName,ei.DesName,ei.EPictureName,ei.EGender,ei.EID,AttStatus,case when DATEDIFF(MINUTE, StartTime , convert(time, InHur+':'+InMin+':'+InSec))<0 then 0 else DATEDIFF(MINUTE, StartTime , convert(time, InHur+':'+InMin+':'+InSec))  end as LateTime from DailyAttendanceRecord dar inner join v_EmployeeInfo ei on dar.EID=ei.EID inner join ShiftConfiguration sftc on dar.ShiftId=sftc.ConfigId where AttStatus in('a','l') and AttDate='" +TimeZoneBD.getCurrentTimeBD( "yyyy-MM-dd") + "'");
         return dt;
     }
     public static DataTable returnSchoolInfo()  // For Load Staff & Faculty
     {

         DataTable dt = new DataTable();
         dt = CRUD.ReturnTableNull("Select * From School_Setup where " +
                    " DSId=(Select Max(DSId) as DSId From School_Setup)");
         return dt;
     }
    }
}
