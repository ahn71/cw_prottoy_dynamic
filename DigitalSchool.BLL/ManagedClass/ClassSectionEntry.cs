...
namespace DS.BLL.ManagedClass
{
    public class ClassSectionEntry : IDisposable
    {
                ...
                if (dt.Rows.Count > 0)
                {
                    ListEntities = (from DataRow row in dt.Rows
                                    select new ClassSectionEntities
                                    {
                                        ...
                                        GroupName = row["GroupName"].ToString(),
                                        ClsGrpID = row["ClsGrpID"].ToString() == "" ? 0 : int.Parse(row["ClsGrpID"].ToString()),
                                        SectionID = int.Parse(row["SectionID"].ToString()),//Privious SectionID here 
                                        SectionName = row["SectionName"].ToString()
        ...
        public static void GetSectionListWithAll(DropDownList dl, int classId, string clsgroupId)
        {
            try
            {
                ClassSectionEntry SectionName = new ClassSectionEntry();
        ...
        public static void GetEntitiesDataWithAll(DropDownList dl, int classId, string clsgroupId)
        {
            try
            {
                ClassSectionEntry SectionName = new ClassSectionEntry();
