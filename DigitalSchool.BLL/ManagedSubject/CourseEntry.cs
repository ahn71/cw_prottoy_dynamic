...
namespace DS.BLL.ManagedSubject
{
    public class CourseEntry
    {
        ...
        public Boolean Insert()
        {
            try
            {
                result = CRUD.ExecuteQuery("insert into AddCourseWithSubject (CourseName,Ordering,SubId,IsActive) values ('" + course_entity.CourseName + "','" + course_entity.Ordering + "','" + course_entity.SubId + "','"+ course_entity.IsActive.ToString()+ "')");
                return result;
            }
            catch { return false; }
        }
        ...
        public Boolean Update()
        {
            try
            {
                result = CRUD.ExecuteQuery("update AddCourseWithSubject set CourseName='" + course_entity.CourseName + "',Ordering='" + course_entity.Ordering + "',SubId='" + course_entity.SubId + "',IsActive='"+ course_entity.IsActive.ToString()+ "' where CourseId =" + course_entity.CourseId + "");
                return result;
            }
            catch { return false; }
        }
        ...
        public static void GetCourseListBySubject(DropDownList ddl,string getSubId)
        {
            try
            {
                ddl.Items.Clear();
                DataTable dt = new DataTable();
                dt = CRUD.ReturnTableNull("select CourseName,CourseId from AddCourseWithSubject where (IsActive is null or IsActive=1) and SubId=" + getSubId + " ");
