...
namespace DS.BLL.ManagedBatch
{
    public class BatchEntry : IDisposable
    {
        ...
        public static void GetDropdownlist(DropDownList dl,int ClassId, bool batchUsed)
        {
            BatchEntry batch = new BatchEntry();
            if (batch.GetEntitiesData() != null)
            {
                batchList = batch.GetEntitiesData().FindAll(c => c.IsUsed == batchUsed && c.ClassId== ClassId).ToList();
                dl.DataValueField = "BatchId";
                dl.DataTextField = "BatchName";
                dl.DataSource = batchList;
                dl.DataBind();
        ...
        public static void GetDropdownlistWithAll(DropDownList dl, string BatchClassId)
        {
            BatchEntry batch = new BatchEntry();
            DataTable dt = batch.GetEntitiesData(BatchClassId);
            dl.DataValueField = "BatchId";
        ...
        public static void GetBatchInfoForExamDependency(DropDownList ddl)
        {
            ...
            + "and BatchId not in (select distinct batchId from v_ExamDependencyInfo  where IsFinal=1) order by BatchId");
