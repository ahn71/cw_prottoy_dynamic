...
namespace DS.BLL.Examinition
{
    public class ExamDependencyInfoEntry
    {
        ...
        public bool Insert()
        {
            try
            {
                result = CRUD.ExecuteQuery("insert into ExamDependencyInfo (ParentExInId,DependencyIExamId,IsFinal) values ('" + _Entities.ParentExInId + "','0','"+_Entities.IsFinal+"')");
                for (byte b = 0; b < _Entities.DependencyIExamId.Items.Count; b++)
                {                         
                    if (_Entities.DependencyIExamId.Items[b].Selected)
                        result = CRUD.ExecuteQuery("insert into ExamDependencyInfo (ParentExInId,DependencyIExamId,IsFinal) values ('" + _Entities.ParentExInId + "','" + _Entities.DependencyIExamId.Items[b].ToString() + "','" + _Entities.IsFinal + "')");                                                    
                }
                return result;
            }
            catch { return false; }
        ...
        public static List<ExamDependencyInfoEntity> GetDependencyExamList
        {
            get
            {
                try
                {
                    ...
                    dt = CRUD.ReturnTableNull("select DeId,ParentExInId+( case when IsFinal=1 then ' [As Final]' else '' end) as ParentExInId from v_ExamDependencyInfo where DependencyIExamId='0'  order by BatchId ");
