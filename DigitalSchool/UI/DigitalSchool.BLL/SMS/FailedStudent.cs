...
namespace DS.BLL.SMS
{
    ...
    {       
        ...
        public List<ExamInfo> GetExamInfoId(string from, string to)
        {
            ...
            sql = string.Format("SELECT [ExInSl],[ExInId],[ExInDate],[BatchName]," +
                                "[ExInDependency] FROM [dbo].[ExamInfo] WHERE Format([ExInDate],'dd-MM-yyyy') " +
                                "BETWEEN '" + from + "' AND '" + to + "'");
                ...
                if (dt.Rows.Count > 0)
                {
                    ListEntities = (from DataRow row in dt.Rows
                                    select new ExamInfo
                                    {
                                        ...
                                        ExInDate = Convert.ToDateTime(row["ExInDate"].ToString()),
                                        Batch = new BatchEntities(){
                                            BatchName = row["BatchName"].ToString()
                                        },
                                        ExInDependency = row["ExInDependency"].ToString()                                                                            
                                    }).ToList();
                    return ListEntities;
                }
