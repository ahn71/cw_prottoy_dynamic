...
namespace DS.BLL.ControlPanel
{
    public class UserAccountEntry :IDisposable
    {
        ...
        public bool Insert()
        {
            try
            {
                result = CRUD.ExecuteQuery("insert into UserAccount (FirstName,LastName,Email,Status,CreatedOn,CreatedBy,ModifiedOn,ModifiedBy,UserName,UserPassword,UserTypeId,EID,IsAdviser)" +
                            ...
                            + _Entities.Status + "','" + _Entities.CreatedOn + "'," + _Entities.CreatedBy + ",'" 
                            + _Entities.ModifiedOn + "'," + _Entities.ModifiedBy + ",'" + _Entities.UserName + "','" 
        ...
        public static void getUserList(GridView gv)
        {
            ...
            try
            {
                DataTable dt = CRUD.ReturnTableNull("select UserId,FirstName,LastName,UserName,UserPassword,Email,CreatedOn,Status,UserType from v_UserAccount");
                gv.DataSource = dt;
                gv.DataBind();
            }
            catch { }
